class TestReportHostFirewall:
	def test_no_rules_added(self, host, add_host_with_interface):
		...
		with open(
			'/export/test-files/report/test_report_host_firewall_no_rules_added.txt'
		) as output:
	...
	def test_nat_rules(self, host, add_host_with_interface, add_network):
		...
		# Do they match what we expect?
		with open(
			'/export/test-files/report/test_report_host_firewall_nat_rules.txt'
		) as output:
			assert result.stdout == output.read()
	...
	def test_drop_rule(self, host, add_host_with_interface):
		...
		# Do they match what we expect?
		with open(
			'/export/test-files/report/test_report_host_firewall_drop_rule.txt'
		) as output:
			assert result.stdout == output.read()
	...
	def test_skip_output_network(self, host, add_host_with_interface):
		...
		# Do they match what we expect?
		with open(
			'/export/test-files/report/test_report_host_firewall_skip_output_network.txt'
		) as output:
			assert result.stdout == output.read()
